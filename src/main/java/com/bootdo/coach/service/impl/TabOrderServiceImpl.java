...
import com.bootdo.coach.api.wx.HttpUtil;
...
public class TabOrderServiceImpl implements TabOrderService {
    ...
    public void dealOrder(TabOrder order, String transaction_id) throws Exception {
        ...
        BigDecimal coachMoney = totalMoney.multiply(coach_devide).setScale(2,BigDecimal.ROUND_DOWN);
        ...
        if (agent0 != null) {
            //一级代理分成
            agent1Money = totalMoney.multiply(bd1.subtract(coach_devide)).multiply(agent_1_divide)
                    .multiply(bd1.subtract(agent_2_divide)).setScale(2,BigDecimal.ROUND_DOWN);
            logger.info("一级代理分：" + agent1Money);
        }
        if (agent1 != null) {
            //二级代理分成
        ...
        if (agent2 != null) {
            ...
            logger.info("三级代理分：" + agent3Money);
        }

        //写入tab_separate_account
        //运营商：60*(1-40%)*(1-20%)
        BigDecimal operatorMoney = totalMoney.multiply(bd1.subtract(coach_devide)).multiply(bd1.subtract(agent_1_divide)).setScale(2,BigDecimal.ROUND_DOWN);
        logger.info("运营商分：" + operatorMoney);
        ...
        // 一级代理分账，失败就回写状态
        if (agent0 != null && agent1Money.compareTo(BigDecimal.ZERO) == 1) {
            ...
            logger.info("一级代理无需分账，信息为空或者可分金额为0");
        }
        // 二级代理分账，失败就回写状态
        if (agent1 != null && agent2Money.compareTo(BigDecimal.ZERO) == 1) {
            ...
            logger.info("二级代理无需分账，信息为空或者可分金额为0");
        }
        // 三级代理分账，失败就回写状态
        if (agent2 != null && agent3Money.compareTo(BigDecimal.ZERO) == 1) {
            ...
            logger.info("三级代理无需分账，信息为空或者可分金额为0");
        }
        ...
        if (topAgentList != null && left_top_money.compareTo(BigDecimal.ZERO) == 1) {
            for (TabTopAgent tabTopAgent : topAgentList) {
                ...
                if (left_top_money.compareTo(separate_money) >= 0) {
                    ...
                    if (result.getIntValue("code") != 1) {
                        ...
                        if (extTopAgentMapper.addAmountDesirable(paramMap) > 0) {
    ...
    public static void main(String[] args) {
        ...
        tabTopAgent2.setDivide(new BigDecimal(30));
