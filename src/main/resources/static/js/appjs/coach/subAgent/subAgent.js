			...
			.bootstrapTable(
					{
						...
						url : prefix + "/subAgentList", // 服务器数据的加载地址
						// showRefresh : true,
						// showToggle : true,
						// showColumns : true,
						iconSize : 'outline',
						...
						columns : [
							...
							},
                            {
                                ...
                                formatter : function(value, row, index) {
                                    if (row.level == 0) {
                                        field: 'agentOneProfit';
                                    } else if (row.level == 1) {
                                        field: 'agentTwoProfit';
                                    }else if (row.level == 2) {
...
//										+ '\')"><i class="fa fa-remove"></i></a> ';
									...
									if (row.isDelete == 0) {
                                        ...
                                        a = '<a class="btn btn-warning btn-sm ' + s_details_h + '" href="#" mce_href="#" title="详情" onclick="details(\''
                                            + row.agentId
...
function details(id) {
    ...
    layer.open({
        ...
        area: ['800px', '90%'],
        content: prefix + '/subDetails/' + id // iframe的url
    });
}
function add() {
	// iframe层
	layer.open({
		...
		area : [ '800px', '90%' ],
		content : prefix + '/subAgentAdd' // iframe的url
	});
}
function remove(id) {
...
function edit(id) {
	layer.open({
		...
		shadeClose : true, // 点击遮罩关闭层
		area : [ '800px', '90%' ],
		content : prefix + '/subAgentEdit/' + id // iframe的url
	});
}
...
function change(id) {
	layer.open({
		...
		shadeClose : true, // 点击遮罩关闭层
		area : [ '800px', '90%' ],
		content : prefix + '/agent1Change/' + id // iframe的url
	});
}
