			...
			.bootstrapTable(
					{
						...
						url : prefix + "/agent2List", // 服务器数据的加载地址
						// showRefresh : true,
						// showToggle : true,
						// showColumns : true,
						iconSize : 'outline',
...
function add() {
	...
	layer.open({
		...
		area : [ '800px', '90%' ],
		content : prefix + '/agent2Add' // iframe的url
	});
}
function remove(id) {
...
function edit(id) {
	layer.open({
		...
		shadeClose : true, // 点击遮罩关闭层
		area : [ '800px', '90%' ],
		content : prefix + '/agent2Edit/' + id // iframe的url
	});
}
...
function change(id) {
	layer.open({
		...
		shadeClose : true, // 点击遮罩关闭层
		area : [ '800px', '90%' ],
		content : prefix + '/agent2Change/' + id // iframe的url
	});
}